...
- Keep the sentence as one sentence. Preserve approximate length; do not summarize or expand.
- Semantic shifts are acceptable, especially if they arise from homophones and/or introduce a touch of absurdity.
- Larger changes (equivalent to two or three small homophonic changes) are acceptable if the result is semantically coherent and/or a bit silly (e.g. "Go ahead, punk: make my day"→"Go ahead, mom: bake my cake").
...
def mutate_chain(sentence: str, steps: int, llm_args: List[str], max_retries: int, prefix: str = "") -> str:
...
def build_tree(root_text: str, depth: int, steps_per_branch: int, llm_args: List[str], max_retries: int) -> Node:
    ...
    def expand(node: Node, path: str = "root"):
        ...
        left_text = mutate_chain(node.text, steps_per_branch, llm_args, max_retries, prefix=f"{path}-L")
        right_text = mutate_chain(node.text, steps_per_branch, llm_args, max_retries, prefix=f"{path}-R")
